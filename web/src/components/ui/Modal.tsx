...
interface ModalProps {
  ...
  /** A wider sheet for content that is itself a picture (the flow card's share preview). */
  wide?: boolean;
}
 ...
 */
export function Modal({ title, onClose, busy = false, children, footer, wide = false }: ModalProps) {
  ...
  return (
    <dialog
      ...
      className={`m-auto ${wide ? "w-[min(46rem,calc(100vw-2rem))]" : "w-[min(32rem,calc(100vw-2rem))]"} rounded-xl border border-gray-200 bg-white p-0 text-gray-900 shadow-2xl backdrop:bg-gray-900/50 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-100`}
    >
