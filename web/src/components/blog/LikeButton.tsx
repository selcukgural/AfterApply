...
export function LikeButton({ postId, initialCount, initialLiked, signInHref }: LikeButtonProps) {
  ...
  const className = `inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-xs font-medium transition-colors disabled:opacity-60 ${
    liked
  ...
  const label = (
    <>
      <svg viewBox="0 0 24 24" className="h-3.5 w-3.5" fill={liked ? "currentColor" : "none"} stroke="currentColor" strokeWidth={2} aria-hidden="true">
