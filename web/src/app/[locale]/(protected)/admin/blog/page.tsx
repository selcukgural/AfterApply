...
import type { AdminBlogPostGroup, AdminBlogPostListItem, BlogLanguage, BlogPostStatus } from "@/types/api";
...
export default function AdminBlogPage() {
  ...
  const list = useQuery({
    queryKey: ["admin", "blog", "grouped", { status, page }],
    queryFn: () => adminBlogApi.listGrouped({ status: status || undefined, page }),
          ...
          <table className="w-full min-w-[56rem] border-collapse text-sm">
            ...
            <tbody>
              {list.data.items.map((group) => (
                <PostRow key={rowKey(group)} group={group} formatDate={formatDate} />
              ))}
            </tbody>
          </table>
        </Card>
...
function PostRow({ group, formatDate }: { group: AdminBlogPostGroup; formatDate: (iso: string) => string }) {
  const t = useTranslations("adminBlog");
  return (
    <tr className="border-b border-gray-100 last:border-b-0 dark:border-gray-800">
      <LanguageCell language="tr" side={group.tr} other={group.en} formatDate={formatDate} />
      <LanguageCell language="en" side={group.en} other={group.tr} formatDate={formatDate} />
...
function LanguageCell({
    ...
    return (
      <td className="px-4 py-3 align-top text-gray-500 dark:text-gray-400">
        {other.translationOfPostId ? (
          ...
          <Link
            href={newTranslationHref(language, other.id)}
  ...
  return (
    <td
      ...
      onClick={() => router.push(`/admin/blog/${side.id}`)}
    >
      <div className="flex flex-col gap-1.5">
        <div>
          <Link href={`/admin/blog/${side.id}`} className="font-medium underline-offset-2 hover:underline" onClick={(e) => e.stopPropagation()}>
