...
import { isVerificationPending } from "@/lib/api/auth";
...
export default function LoginPage() {
  ...
  // Set when the password was right but the address was never verified (2026-09-24).
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      const outcome = await login(result.data);
  ...
  return (
    <div className="flex flex-1 items-center justify-center px-4 py-12">
      <div className="w-full max-w-sm rounded-lg border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900">
        {pending ? (
          <EmailVerificationStep pending={pending} onVerified={finishSignIn} />
        ) : (
          <>
