...
export async function generateMetadata({ params }: PageProps<"/[locale]/help/candidate-experiences">): Promise<Metadata> {
  const { locale } = await params;
  return pageMetadata(locale, "/help/candidate-experiences", "helpCandidateExperiences");
}

export default async function CandidateExperiencesHelpPage({ params }: PageProps<"/[locale]/help/candidate-experiences">) {
  const { locale } = await params;
  setRequestLocale(locale);
  const t = await getTranslations("help.candidateExperiences");
  ...
  return (
    <div className="flex flex-col gap-10">
      <HelpBreadcrumbJsonLd path="/help/candidate-experiences" />
      ...
      <section className="flex flex-col gap-4">
        ...
        <Screenshot src="/help/screenshots/company-experiences.png" alt={t("read.title")} />
      </section>
      ...
      <section className="flex flex-col gap-2">
        <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">{t("proof.title")}</h2>
        <p className="text-sm leading-6 text-gray-600 dark:text-gray-400">{t("proof.body")}</p>
      </section>
