...
export async function generateMetadata({ params }: PageProps<"/[locale]/help/settings">): Promise<Metadata> {
  const { locale } = await params;
  return pageMetadata(locale, "/help/settings", "helpSettings");
}

export default async function SettingsHelpPage({ params }: PageProps<"/[locale]/help/settings">) {
  const { locale } = await params;
  ...
  return (
    <div className="flex flex-col gap-10">
      <HelpBreadcrumbJsonLd path="/help/settings" />
