...
public class GoogleSignupTokenTests
{
    ...
    public async Task Round_Trips_The_Identity()
    {
        ...
        var restored = await service.ValidateGoogleSignupTokenAsync(token);

        restored.ShouldBe(Identity);
    }

    [Fact]
    public async Task Round_Trips_An_Identity_Without_Names()
    {
        var service = Service();
        var identity = Identity with { GivenName = null, FamilyName = null };

        (await service.ValidateGoogleSignupTokenAsync(service.CreateGoogleSignupToken(identity))).ShouldBe(identity);
    }
    ...
    public async Task Rejects_A_Tampered_Token()
    {
        ...
        (await service.ValidateGoogleSignupTokenAsync(string.Join('.', parts))).ShouldBeNull();
    }
    ...
    public async Task Rejects_An_Expired_Token()
    {
        ...
        (await service.ValidateGoogleSignupTokenAsync(token)).ShouldNotBeNull();
    ...
    public async Task An_Access_Token_Is_Not_A_Signup_Token()
    {
        ...
        (await service.ValidateGoogleSignupTokenAsync(accessToken)).ShouldBeNull();
    }

    [Fact]
    public async Task Garbage_Is_Rejected_Not_Thrown()
    {
        (await Service().ValidateGoogleSignupTokenAsync("nope")).ShouldBeNull();
    }
