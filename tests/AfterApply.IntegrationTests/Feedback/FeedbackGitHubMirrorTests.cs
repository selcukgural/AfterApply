...
/// <summary>Turns the mirror back on for this class only. TestContainerCleanup forces it off
/// process-wide through environment variables, which sit above user secrets — the whole point
/// being that a developer's live token can never reach a test host. An in-memory source added
/// here is appended last, so it beats those variables.</summary>
public sealed class FeedbackGitHubMirrorProfile : IHostProfile
{
