...
public class BulkOperationTests(ApiHost<DefaultProfile> host) : IClassFixture<ApiHost<DefaultProfile>>, IAsyncLifetime
{
    ...
    private async Task<HttpClient> SignUpAsync(string email)
    {
        ...
        var auth = await TestAccounts.RegisterVerifiedAsync(client, _factory!.Services,
            new RegisterRequest(email, "P@ssw0rd123!", "Bulk", "Tester", true));
