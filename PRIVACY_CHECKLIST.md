...
| **İK kontağı (2026-09-06, isteğe bağlı)** — kullanıcının bir başvuru/takip edilen ilan için kendi elleriyle girdiği ad, e-posta ve LinkedIn profil adresi | Cloud SQL (`Applications`/`TrackedJobs` üzerinde `HrName`/`HrEmail`/`HrLinkedInUrl`). Kullanıcı başına saklanıyor — paylaşımlı `Job`/`Company` satırında **değil** — dolayısıyla başvuru silinince veya hesap silinince birlikte siliniyor, ve başka bir kullanıcıya hiçbir koşulda görünmüyor. `CompanyIntelligence`'ın kullanıcılar arası agregasyonlarına girmiyor. | Hayır. Sistem bu adrese kendiliğinden hiçbir şey göndermiyor — arayüzde yalnızca bir `mailto:` bağlantısı |
