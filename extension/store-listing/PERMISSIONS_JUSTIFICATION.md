...
```
This extension lets a signed-in e-kariyerim user save the job posting they are currently viewing
on LinkedIn or kariyer.net as a tracked application in their own e-kariyerim account, with one
click.
```
...
personal access token, and looks up existing company names for the autocomplete field. No other
